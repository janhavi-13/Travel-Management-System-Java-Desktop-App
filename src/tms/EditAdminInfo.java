...
public class EditAdminInfo extends JFrame implements ActionListener,DocumentListener{
    ...
    EditAdminInfo(String username){
        ...
        ImageIcon i = new ImageIcon(ClassLoader.getSystemResource("tms/icons/editInfo.jpg"));
        ...
        c1.add("What was the model of your first vehicle?");
        c1.add("What is your exact birth time?");
        c1.add("What is your mothers maiden name?");
        ...
        try{
            Conn c =new Conn();
            ResultSet rs = c.s.executeQuery("select * from password where userName= '"+username+"'");
            while(rs.next()){
                f1.setText(rs.getString("userName"));     //get text from username column and put it in textField 
                f2.setText(rs.getString("name"));
                f3.setText(rs.getString("sAnswer"));
                c1.select(rs.getString("sQuestion"));
            }
        }catch(Exception e){}
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b1){
            ...
            String sql = "update password set userName ='"+userName+"',name ='"+name+"',sQuestion ='"+question+"',sAnswer ='"+answer+"'";
            try{
                Conn c =new Conn();
