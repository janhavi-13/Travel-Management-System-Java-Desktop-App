...
public class PackageReport extends JFrame implements ActionListener{
     ...
     PackageReport(String username){
        ...
        JLabel l = new JLabel("Package  Name :");
        ...
        b2 = new JButton("All Packages Details");
        ...
        b3 = new JButton("Check Package Availability");
        ...
        b5 = new JButton("View Fully Booked Packages");
     ...
     public void actionPerformed(ActionEvent e){
        ...
        if(e.getSource()==b2){
            Conn c = new Conn();
            try{
                JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\PReport.jrxml");
                String sql = "select * from addPackage";
        ...
        if(e.getSource()==b3){
            Conn c = new Conn();
            try{
                JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\BAReport.jrxml");
        ...
        if(e.getSource()==b1){
            Conn c = new Conn();
            try{
                JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\PNameReport.jrxml");
                String sql = "select * from addPackage where pName = '"+c1.getSelectedItem()+"'";
                JRDesignQuery updateQuery = new JRDesignQuery();
                updateQuery.setText(sql);
                jd.setQuery(updateQuery);
                JasperReport jreport = JasperCompileManager.compileReport(jd);
        ...
        if(e.getSource()==b5){
            Conn c = new Conn();
            try{
                JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\FBPReport.jrxml");
                String sql = "select * from bCapacity where available=0";
                JRDesignQuery updateQuery = new JRDesignQuery();
                updateQuery.setText(sql);
                jd.setQuery(updateQuery);
                JasperReport jreport = JasperCompileManager.compileReport(jd);
