...
public class ViewHotel extends JFrame implements ActionListener{
    ...
    ViewHotel(String username,String hName){
        ...
        try{
            Conn c1 = new Conn();
            ResultSet rs = c1.s.executeQuery("select hName from customerDetails");
            while(rs.next()){
                if(rs.getString("hName").equals(hName)){
                    b1.setEnabled(false);
                    b2.setEnabled(false);
                    JLabel l28 = new JLabel("Can't update/remove hotel as bookings are already started");
    ...
    public void actionPerformed(ActionEvent e){
        Conn c = new Conn();
        if(e.getSource()==b1){
            new UpdateHotel(username,hName).setVisible(true);
            setVisible(false);
        }
        else if(e.getSource()==b2){
            int j=0,k=0;
            //System.out.println(j);
            String rem = "delete from Hotel where hName='"+hName+"'";
            String sr2 = "select SrNo from Hotel where hName='"+hName+"'";
            String sr3 = "select count(hName) from Hotel";
          
            //reset auto increment
            try{
                ResultSet rs2 = c.s.executeQuery(sr3);
                while(rs2.next()){
                    k=Integer.parseInt(rs2.getString("count(hName)"));
                }
