...
public class CDReport extends JFrame implements ActionListener,MouseListener,DocumentListener{
    String username;
    JButton b1,b4,b2;
    JTextField t1;
    ...
    CDReport(String username){
       ...
       // jdc.setMinSelectableDate(d);
        ...
        b2 = new JButton("All Customer Details");
    ...
    public void actionPerformed(ActionEvent e){
            ...
            try{
                if(!t1.getText().equals("")  && ((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && ((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    ...
                    if(Integer.parseInt(t1.getText()) > count){
                        ...
                        JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\cdReport.jrxml");
                        String sql = "select * from customerDetails where SrNo='"+t1.getText()+"'";
                ...
                if(t1.getText().equals("")  && !((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && ((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    ...
                    if(count==0){
                        ...
                        JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\dateReport.jrxml");
                        String sql = "select * from customerDetails where datee='"+date+"'";
                        JRDesignQuery updatequery = new JRDesignQuery();
                        updatequery.setText(sql);
                        jd.setQuery(updatequery);
                        JasperReport jreport = JasperCompileManager.compileReport(jd);
                ...
                if(t1.getText().equals("")  && !((JTextField)jdc.getDateEditor().getUiComponent()).getText().equals("") && !((JTextField)jdc1.getDateEditor().getUiComponent()).getText().equals("")){
                    //select * from customerDetails where datee >= "2022-04-08" && datee < "2022-04-12";
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                    ...
                    int count = 0;
                    ResultSet rs3 = c.s.executeQuery("select count(fName) from customerDetails where datee >= '"+date1+"' && datee <= '"+date2+"'");
                    if(rs3.next()){
                        count = Integer.parseInt(rs3.getString("count(fName)"));
                    }    
                    if(count==0){
                        UIManager ui = new UIManager();
                        ui.put("OptionPane.background",Color.WHITE);
                        ...
                        f.setVisible(true);
                        JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\TwoDatesReport.jrxml");
                        String sql = "select * from customerDetails where datee >= '"+date1+"' && datee <= '"+date2+"'";
                        JRDesignQuery updatequery = new JRDesignQuery();
                        updatequery.setText(sql);
                        jd.setQuery(updatequery);
                        JasperReport jreport = JasperCompileManager.compileReport(jd);
        ...
        if(e.getSource()==b2){
            Conn c = new Conn();
            try{
                JasperDesign jd = JRXmlLoader.load("G:\\TMS\\src\\tms\\reports\\report1.jrxml");
